...
func TestConvertOptionalContainerConfigToScannerConfig(t *testing.T) {
	...
	if err := Convert_v1alpha2_OptionalContainerConfig_To_unversioned_ScannerConfig(&in, &out, nil); err != nil {
	...
	if out.Windows != nil {
		t.Error("Windows must be nil: v1alpha2 has no per-OS override to convert from")
	}
}
...
func TestConvertScannerConfigToOptionalContainerConfigDropsWindows(t *testing.T) {
	...
	if err := Convert_unversioned_ScannerConfig_To_v1alpha2_OptionalContainerConfig(&in, &out, nil); err != nil {
		t.Fatalf("convert: %v", err)
	}

	// v1alpha2 cannot express the override, so the Linux side must survive
